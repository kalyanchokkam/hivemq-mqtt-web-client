...
var websocketclient = {
    ...
    'onConnect': function () {
        ...
        websocketclient.subscribe('temperature',2,12312);
        websocketclient.subscribe('snsStatus',2,56465);
        websocketclient.subscribe('screenshot/#',2,4)
    },
...
//        console.log("onMessageArrived:" + message.payloadString + " qos: " + message.qos);
        ...
        var messageObj = {};
        if (message.destinationName.includes('screenshot')) {
            messageObj = {
    ...
    'publish': function (topic, payload, qos, retain) {
        ...
        for (var i = 0; i < topicList.length; i++ ){
            ...
            message.destinationName = topicList[i];
            message.qos = qos;
    ...
    'render': {
        ...
        'message': function (message) {
            ...
            var topic = message.topic.replace("/","");
            if (topic.includes("screenshot")) {
                //var blob = new Blob( message.payloadBytes, { type: "image/jpeg" } );
                // var imageUrl = URL.createObjectURL( blob );
                var imgsrc = "data:image/png;base64," + btoa(String.fromCharCode.apply(null,message.payload));

                var imgHtml = '<img src="'+imgsrc+'" alt='+'"image"'+'>';
                $("#screenshots").prepend(imgHtml);
                return largest;
            }
            var html = '';
            if (topics.includes(topic)){
                html = '<span class="msgElements">'+ Encoder.htmlEncode(message.payload) + '</span>';
                ...
                html = '<div class="large-12">'+topic+'</div>' +
                    '<div id="'+topic+ '" class="large-12 msgblock">' +
                    '<span class="msgElements">'+ Encoder.htmlEncode(message.payload) + '</span>'+
                    '</div>';
                //  html = '<ul id="'+topic+'" class="large-3" ' +
                //     '<li class="messLine id="' + largest + '">' +
                //     '   <div class="row large-12 mess' + largest + '" style="border-left: solid 10px #' + message.color + '; ">' +
                //     '       <div class="large-12 columns messageText">' +
                //     '           <div class="large-3 columns date">' + message.timestamp.format("YYYY-MM-DD HH:mm:ss") + '</div>' +
                //     '           <div class="large-5 columns topicM truncate" id="topicM' + largest + '" title="' + Encoder.htmlEncode(message.topic, 0) + '">Topic: ' + Encoder.htmlEncode(message.topic) + '</div>' +
